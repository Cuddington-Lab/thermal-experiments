...
datin <- subset(datin, !Frond_count == 0)
...
library(lme4)
lm.fit.spF = lm(Frond_count ~ label*as.factor(Mean_temperature),  data=datin.w)
experimF <- lmer(Frond_count ~ label*as.factor(Mean_temperature) + (1|Experiment_Number),  data=datin.w)
incubF <- lmer(Frond_count ~ label*as.factor(Mean_temperature) + (1|Incubator),  data=datin.w)
inc_expF <- lmer(Frond_count ~ label*as.factor(Mean_temperature) + (1|Incubator)+(1|Experiment_Number),  data=datin.w)
...
ggplot(data=datin.w, aes(x=factor(Mean_temperature), y=Frond_count, color = label, group = label)) +
  geom_jitter(aes(), size=4,width = 0.1, height = 0.1) +
  geom_smooth(method=lm,se=F,size=2, formula=y~x, fullrange=TRUE) +
...
library(emmeans)
f <- emmeans(experimF, list(pairwise ~ label*Mean_temperature), lmer.df="satterthwaite")
...
plot(resid(experimF),datin.w$Frond_count)
...
fligner.test(residuals(experimF) ~ datin.w$Experiment_Number)
fligner.test(residuals(experimF) ~ datin.w$Mean_temperature)
