...
lines(tseq[,4], type="l", col="blue")

#if the data is not within desired autocorlated coefficients (0, 0.95) run again 
if(round(acz[1],2)==0&&round(acz[3],2)==0.95){
  b = "yes"
}
# prints out the autocorlation vaules (mostly for debug purpose) 
print(round(acz[1],2))
print(round(acz[3],2))
...
tempfilename=paste("C:/Users/Cuddington_admin/Documents/AphidProfiles/temps_mean_",round(mtemp,0),"sd_",
                   round(sdev*10,0),"seed_",a,".csv", sep="")
write.csv(temps*10,tempfilename, row.names=FALSE)

#tempfilename=paste("C:/Users/Cuddington_admin/Documents/AphidProfiles/temps_mean_"sd_",round(sdev*10,0),".csv", sep="")
#write.csv(temps*10,tempfilename, row.names=FALSE)

#randomize treatment assignment (to be sequentially matched with incubators 2, 3, and 4)
#1= zero
#2= 0.95
#3= 0.95rev
sample(1:3)
