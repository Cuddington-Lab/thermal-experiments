...
# Logistic2 Model
logistic2 <- function(temp, a, b, c, d) {
  a / (1 + exp(-b * temp)) * (1 - exp(c * (d - temp)))
}
...
modified_gaussian <- function(temp, a, b, c) {
  a * exp(-0.5 * ((temp - b) / c)^2)
}

# Exponential Modified Gaussian Model
exp_modified_gaussian <- function(temp, a, b, c) {
  exp(0.5 * (b / a)^2 - (temp - c) / a) * (erf((1 / sqrt(2)) * ((c - temp) / b) - (b / a)) +
                                             erf((1 / sqrt(2)) * ((temp - c) / b) - (b / a)))
}
...
# Read dataset
tpc <- read.csv("https://raw.githubusercontent.com/Cuddington-Lab/thermal-experiments/main/autocorrelation_TPC.csv", 
...
# First plot
plot(
  "",  # Just plotting a point to initialize
...
# Add legend
legend(
  "bottomleft", legend = unique(tpc$Treatment), 
  col = colors, lwd = 3, cex = 2, bty = "n"
)
...
for (treatment in unique(tpc$Treatment)) {
  ...
  cubic_ratkowsky_fit <- tryCatch({
    nls_multstart(
      frond ~ cubic_ratkowsky(temp, a, b, tmin, tmax),
      data = tpc_treatm,
  ...
  # Fit Briere 2 model
  briere2_fit <- tryCatch({
    nls_multstart(
      frond ~ briere2(temp, a, tmin, tmax, b),
      data = tpc_treatm,
      start_lower = list(a = 0.0001, tmin = 0, tmax = 30, b = 1),
      start_upper = list(a = 1, tmin = 10, tmax = 40, b = 5),
      iter = 100, supp_errors = 'Y', convergence_count = FALSE
    )
  }, error = function(e) {
    return(NULL)  
  ...
  performance_fit <- tryCatch({
    nls_multstart(
      frond ~ performance(temp, k, a, m, b, n),
      data = tpc_treatm,
      start_lower = list(k = 0.05, a = 0.0005, m = 10, b = -0.2, n = 30),
      start_upper = list(k = 0.5, a = 0.01, m = 20, b = 0.2, n = 35),
  ...
  # Fit Logistic2 Model
  logistic2_fit <- tryCatch({
    nls_multstart(
      frond ~ logistic2(temp, a, b, c, d),
      data = tpc_treatm,
      start_lower = list(a = 0.01, b = 0.01, c = 0.01, d = 30),
      start_upper = list(a = 10, b = 10, c = 10, d = 40),
      iter = 200, supp_errors = 'Y', convergence_count = FALSE
  ...
  # Fit Modified Gaussian Model
  modified_gaussian_fit <- tryCatch({
    nls_multstart(
      frond ~ modified_gaussian(temp, a, b, c),
      data = tpc_treatm,
      start_lower = list(a = 0.01, b = 10, c = 30),
      start_upper = list(a = 10, b = 50, c = 50),
      iter = 100, supp_errors = 'Y', convergence_count = FALSE
    )
  }, error = function(e) {
    return(NULL)  
  ...
  # Fit Exponential Modified Gaussian Model
  exp_modified_gaussian_fit <- tryCatch({
    nls_multstart(
      frond ~ exp_modified_gaussian(temp, a, b, c),
      data = tpc_treatm,
      start_lower = list(a = 0.01, b = 1, c = 30),
      start_upper = list(a = 10, b = 5, c = 50),
      iter = 100, supp_errors = 'Y', convergence_count = FALSE
    )
  }, error = function(e) {
    return(NULL)  
  ...
  fit_list <- list(
    Newfunction = cubic_ratkowsky_fit,
    Hinshelwood = hinshelwood_fit,
    Quadratic = quadratic_fit,
    Briere2 = briere2_fit,
    Performance = performance_fit,
    Logistic1 = logistic1_fit,
    Logistic2 = logistic2_fit,
    Sharpe_Schoolfield = sharpe_schoolfield_fit,
    Ratkowsky = ratkowsky_fit,
    Briere1 = briere1_fit,
    Weibull = weibull_fit,
    Modified_Gaussian = modified_gaussian_fit,
    Exp_Modified_Gaussian = exp_modified_gaussian_fit,
    Eppley = eppley_fit
  )
  ...
  if (!is.null(best_model)) {
    ...
    points(tpc_treatm$temp + rnorm(nrow(tpc_treatm), mean = 0, sd = 0.4),
           tpc_treatm$frond, pch = 1, cex = 3, col = colors[color_index], lwd = 2)
  }
...
mtext("Temperature (B0C)", side = 1, line = 2.8, cex = 1.8)
...
jittered_temp <- tpc_hot_cold$Mean_temperature + rnorm(length(tpc_hot_cold$Mean_temperature), mean = 0, sd = 0.4)
