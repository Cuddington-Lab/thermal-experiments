...
## View dataset and response variable (r = relative growth rate)
```{r echo=FALSE}
...
# datin$total_living_fronds[datin$total_living_fronds == 0] <- 0.001
# datin$r <- (log(datin$total_living_fronds) - log(12)) / 5
# datin$r[datin$r < 0] <- 0
...
create_histogram <- function(data, temp, label) {
  ggplot(subset(data, Mean_temperature == temp & Label == label), 
    ...
    geom_histogram(colour = "#999999", bins = 10, position = "stack") + 
    theme_bw(base_size = 16) + 
