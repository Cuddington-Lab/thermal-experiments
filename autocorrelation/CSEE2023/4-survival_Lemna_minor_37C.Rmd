...
library(stringr)
datin3$label <- str_replace(datin3$label, "0N/A", " no autocorrelation")
datin3$label <- str_replace(datin3$label, "0.95N", "hot-cold")
datin3$label <- str_replace(datin3$label, "0.95P", "cold-hot")

datin3 <- subset(datin3, !Species == "LP")

library(ggplot2)
ggplot(datin3, aes(x = label, y = survival, color = label)) + 
  geom_jitter() +
  labs(title = "L. minor survival") +
  xlab("treatment") +
...
glm.fit.sp = glm(survival ~ label, family = binomial, data=datin3)
experim <- glmer(survival ~ label + (1|Experiment_Number), family=binomial, data=datin3)
incub <- glmer(survival ~ label + (1|Incubator), family=binomial, data=datin3)
dish <- glmer(survival ~ label + (1|dish), family=binomial, data=datin3)
experim_inc_dish <- glmer(survival ~ label + (1|Experiment_Number) + (1|Incubator) + (1|dish), family=binomial, data=datin3)
inc_dish <- glmer(survival ~ label + (1|Incubator)+(1|dish), family=binomial, data=datin3)
inc_exp <- glmer(survival ~ label + (1|Incubator)+(1|Experiment_Number), family=binomial, data=datin3)
dish_exp <- glmer(survival ~ label + (1|dish)+(1|Experiment_Number), family=binomial, data=datin3)
exp.inc_dish <- glmer(survival ~ label + (1|Experiment_Number/Incubator) + (1|dish), family=binomial, data=datin3)
exp.inc <- glmer(survival ~ label + (1|Experiment_Number/Incubator), family=binomial, data=datin3)
...
library(ggeffects)
ggpredict(exp.inc, "label",colors = "label") %>% plot(dot.size = 5,line.size = 2.5,colors = "label") + 
...
pairwise <- emmeans(exp.inc, list(pairwise ~ label), lmer.df="satterthwaite")
pairwise
pairs <- as.data.frame(pairwise$`pairwise differences of label`)
